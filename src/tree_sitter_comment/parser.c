...
{
  if (!tscomment_is_upper(lexer->lookahead) || !valid_symbols[T_TAGNAME])
  {
    return false;
  }
  ...
  while (tscomment_is_upper(lexer->lookahead) || tscomment_is_digit(lexer->lookahead) || tscomment_is_internal_char(lexer->lookahead))
  {
  ...
  if (tscomment_is_internal_char(previous))
  {
    return false;
  }
  ...
  if ((tscomment_is_space(lexer->lookahead) && !tscomment_is_newline(lexer->lookahead)) || lexer->lookahead == '(')
  {
    // Skip white spaces.
    while (tscomment_is_space(lexer->lookahead) && !tscomment_is_newline(lexer->lookahead))
    {
      lexer->advance(lexer, false);
    }
    // Checking aperture.
    ...
    {
      if (tscomment_is_newline(lexer->lookahead))
      {
        return false;
      }
      lexer->advance(lexer, false);
  ...
  lexer->advance(lexer, false);
  if (!tscomment_is_space(lexer->lookahead))
  {
    return false;
  }
